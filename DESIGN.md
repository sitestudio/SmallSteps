...
**Light Mode - Text Area (Expanded Checklist Item)**
- Background: `#f1f4f7` - cool light gray for contrast against white items
- Text: `#1c2b33` - dark charcoal for readability
- Checkbox checkmark: `#fbbd41` (Lemon 500) - warm gold consistent with button styling

**Dark Mode - Text Area (Expanded Checklist Item)**
- Background: `var(--dark-surface)` or `#2d3038` - dark surface color
- Text: `var(--dark-text)` or `#f1f4f7` - light text for readability
- Checkbox checkmark: `#fbbd41` (Lemon 500) - maintains consistency across modes

**Interactive States**
- Hover: background shifts ~2% (lighter in light mode, darker in dark mode)
- Focus: `rgb(20, 110, 245) solid 2px` outline for accessibility
- Disabled: opacity reduced to 60% with muted text color
...
**Light Mode (Default)**
- Page background: `#faf9f7` - warm cream canvas
- Text color: `#000000` or `#1c2b33` - Clay Black/dark charcoal
- Surface: `#ffffff` or `var(--white)`

**Dark Mode**
- Page background: `#1c1e21` - Clay Black (`--dark-bg`)
- Text color: `#f1f4f7` - Warm Silver (`--dark-text`)
- Surface: `#2d3038` - Dark surface (`--dark-surface`)

These variables are defined in `src/styles.scss`:
...
The application uses consistent button patterns to maintain visual harmony:

**Primary Action Buttons (Pill Shape)**

- Border radius: 100px (rounded pill shape, using the large Pill value from border-radius scale)
- Background: Lemon 500 (`#fbbd41`) - warm gold color from swatch palette
- Text: white (`#ffffff`)
- Hover: background shifts to Lemon 700 (`#d08a11`), subtle scale (1.05)
- Padding: 14px vertical, 32px horizontal
- Font: Roobert or Montserrat 16px weight 500

**Three-Button Action Row Pattern**

- Used on: sounds-speech pages (words-and-sentences, sounds-speech-training)
- Layout: Horizontal row with 16px gap between buttons
- Order: Back (left) | Primary Action (center) | Home (right)
- All three buttons use pill styling with 100px border-radius
- Back button: uses same primary styling (Lemon 500 background, white text)
- Home button: uses same primary styling
- Each button includes navigation icons (← for back, → for home) for clarity

**Dark Mode Adjustments**

- Background color remains Lemon 500 (`#fbbd41`) - the warm gold adapts to dark context
- Text remains white (`#fbbd41` for background, `#000000` or `#ffffff` for contrast)
- Border: 1px solid `#3d414a` (dark slate) on dark mode for buttons with border
